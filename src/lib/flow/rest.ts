 ...
 *
 * **업무 판정은 `taskStatus`가 채워졌는가로 한다.** `templateType`도 갈리기는 하지만 문서가
 * 이 값을 `"1"` 하나로 적어 두고 실제로는 **7종**이 오는 필드다 (§6.2) — 목록으로 짜면 종류가
 * 늘 때 그대로 새어 나온다. **실제로 새어 나왔다**: 이 주석을 처음 쓸 때 관측된 건 넷
 * (`92`·`4`·`91`·`93`)이었는데 3,132건을 다시 훑으니 `1`(일반글 구)·`2`(할일)·`3`(일정 구)
 * 33건이 더 나왔다. 화이트리스트였다면 그 33건이 통째로 사라졌을 것이다. `taskStatus`는 업무
 * 2,402건 다 채워 오고 나머지 730건은 전부 빈 문자열이라 한 건도 안 겹친다
 * `(실측 2026-08-06, 프로젝트 59 / 글 3,132건)`.
 *
