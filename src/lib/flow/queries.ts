...
/**
 * 10줄을 채우려고 확인해 보는 픽 수. 여기까지만 댓글을 본다 — 픽마다 REST 1회라
 * 상한(분당 120회)에서 이 숫자가 곧 비용이다.
 *
 * `FOCUS_LIMIT`의 1.6배로 둔다(5:8이던 비율 그대로). 아래에서 피드백 픽이 떨어져 나가는데
 * 여유가 없으면 10줄이 안 찬다. 응답은 `FOCUS_COMMENT_TTL`(5분) 동안 캐시에 남아서 같은
 * 화면을 다시 열 때는 안 나간다.
 */
const FOCUS_CHECK = 16;

/** 댓글 응답을 데이터 캐시에 두는 시간(초). 같은 화면을 다시 열어도 안 부른다. */
const FOCUS_COMMENT_TTL = 300;

/**
 * 포커스 10줄. **피드백 상태인데 마지막 댓글을 내가 쓴 업무는 내린다.**
 *
