 ...
 *
 * PRD는 프로젝트 59개를 각각 조회해 롤업하라고 했지만, `flow_get_team_standup`이
 * 부서원 전원의 임박/밀림 업무를 **한 번에** 준다. 그 업무들을 프로젝트 이름으로 묶으면
 * 같은 순위표가 MCP 호출 1회로 나온다 — 59회가 아니라.
 *
 * 대신 범위가 "선택한 부서가 담당한 업무"로 좁혀진다. 회사 전체 프로젝트 리스크가 아니다.
