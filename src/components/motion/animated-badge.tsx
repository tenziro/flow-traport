...
// PRD §7.1 토큰으로 교체 (원본 beUI는 emerald/amber 원색을 직접 쓴다 — 색 체계가 둘이 되면 안 된다).
// 텍스트는 -foreground 파생값. 원색 #F59E0B는 흰 배경에서 2.1:1이라 텍스트로 못 쓴다.
const STATUS_CLASS: Record<AnimatedBadgeStatus, string> = {
  neutral: "border-border bg-neutral-bg text-neutral-foreground",
  info: "border-primary/30 bg-primary/10 text-primary",
  success: "border-success/30 bg-success-bg text-success-foreground",
  warning: "border-warning/40 bg-warning-bg text-warning-foreground",
  danger: "border-danger/30 bg-danger-bg text-danger-foreground",
...
const ICONS: Record<AnimatedBadgeStatus, ComponentType<IconProps>> = {
  neutral: IconDot,
  info: IconInfo,
  success: IconNormal,
  warning: IconRisk,
