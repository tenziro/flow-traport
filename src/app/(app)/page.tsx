...
export const metadata = { title: "오늘 · flow 콕핏" };
...
export default async function TodayPage({
  ...
  const mentions = groupMentions(worklist.mentions);
   ...
   */
  const shownFocus = params.focus ? focus?.filter((p) => p.status === params.focus) : focus;
  ...
  const load = counts.imminent + counts.overdueActive + mentions.length + counts.overdueStale;
    ...
    <>
      <header className="rise mb-3 flex flex-wrap items-end justify-between gap-x-6 gap-y-1">
        ...
        <p className="tabular text-xs text-muted-foreground">
          챙길 일 <span className="text-sm font-semibold text-foreground">{load}</span>건
        </p>
      </header>

      {/* KPI는 한 줄로 세운다. 2×2로 접으면 4칸이 두 덩어리로 보여서 순위가 안 읽힌다 */}
      <section aria-label="요약" className="mb-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
        <Stat
          i={1}
          label="마감 임박"
        ...
        <Stat
          ...
          tone="danger"
          note={
            delay[2].value > 0 ? `8일 이상 밀린 게 ${delay[2].value}건이에요` : "마감이 지났어요"
          }
        />
        <Stat
          i={3}
      ...
      <div className="mb-3 grid items-start gap-3 xl:grid-cols-12">
         ...
         * 건수는 아래에 그대로 밝힌다.
         */}
        <Card className="rise xl:col-span-4" style={{ "--i": 6 } as React.CSSProperties}>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
          ...
          <CardContent className="space-y-0.5">
            {counts.overdueStale === 0 ? (
              <EmptyState icon={<IconStale size={18} />} title="방치된 업무가 없어요" />
            ) : stale === null ? (
              ...
              <>
                {stale.map((task, i) => (
                  ...
                  >
                    <TaskItem task={task} projectId={idOf(task.project)} path="/" />
                  </div>
                ))}
                {stale.length < counts.overdueStale && (
                  <p className="tabular px-2 pt-2 text-xs text-muted-foreground">
                    180일 넘게 손 안 댄 {counts.overdueStale - stale.length}건은 flow에서 직접
                    확인해요.
                  </p>
                )}
              </>
      ...
      <div className="grid items-start gap-3 xl:grid-cols-12">
        ...
        >
          ...
          <CardContent className="space-y-0.5">
            {worklist.overdueActive.length === 0 ? (
              <EmptyState icon={<IconRisk size={18} />} title="밀리는 업무가 없어요" />
            ) : (
        ...
        <Card className="rise xl:col-span-4" style={{ "--i": 8 } as React.CSSProperties}>
          <CardHeader>
          ...
          <CardContent>
            {mentions.length === 0 ? (
              <EmptyState icon={<IconMention size={18} />} title="새로 부른 사람이 없어요" />
            ) : (
              ...
              <div>
                {mentions.map((group) => (
                  ...
                  <details
                    key={group.taskId}
                    className="group rounded-lg px-2 py-1.5 transition-colors duration-300 ease-out hover:bg-muted"
                  >
                    <summary className="flex cursor-pointer list-none items-start gap-2 text-sm">
                      ...
                      <span className="min-w-0 flex-1">
                        {/* 본문은 댓글보다 한 급 크고 진하게 — 같은 크기면 첫 댓글이 제목처럼 읽힌다 */}
                      ...
                      <ul className="space-y-3">
                        {[...group.alarms].reverse().map((alarm, i) => (
                          <li
                            ...
                            className={cn("flex gap-2", alarm.isReply && "ml-5")}
                          >
...
function Stat({
  ...
  return (
    <Card size="sm" className="rise gap-2" style={{ "--i": i } as React.CSSProperties}>
      <CardContent className="space-y-1.5">
        ...
        <p className="flex items-baseline gap-1">
          ...
          <span className="tabular text-xs text-muted-foreground">/ {total}건</span>
        </p>
        <Meter total={total} segments={[{ value, label, className: BAR[tone] }]} />
