...
버전 3.0.2 · 2026-08-04 기준. 로드맵 정의는 [PRD.md](PRD.md) §11에 있다.
...
곁가지로 업무 필터 응답의 **등록자(글 작성자) 컬럼을 실측했다** — `RGSR_ID`는
`columnType: USER`고 `userName`에 실명이 온다(프로젝트 4곳 100%, 담당자 없는 프로젝트에도
있었다). `/tasks`·`/risk`는 이 응답을 이미 쓰니 호출 없이 작성자 열을 켤 수 있고, 오늘·팀
화면은 MCP 응답에 없어서 업무마다 REST 1회가 더 붙는다 (등록일 열과 같은 사정).
[api-spec.md](api-spec.md) §6.1에 적었다.
