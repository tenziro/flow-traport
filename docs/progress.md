...
버전 4.17.0 · 2026-08-07 기준. 로드맵 정의는 [PRD.md](PRD.md) §11에 있다.
...
| 18 | 멘션 모달과 헤더 알림을 업무 상세 모달로 | 완료 (v4.9.0 · v4.10.0) |
| 19 | 검색이 넷을 찾고, 줄마다 다른 곳으로 데려간다 | 완료 (v4.11.0) |
| 20 | 새 소식이 오면 OS 알림 — 폴링에 얹은 알림 | 완료 (v4.12.0) |
| 21 | 오늘 일정 띠 · 답글은 그 자리에 · 폰에서는 탭이 고르개 | 완료 (v4.13.0) |
| 22 | 로딩 골격을 지금 화면에 맞춰 현행화 | 완료 (v4.13.1) |
| 23 | 화면 대기 시간 실측 후 프로젝트 훑기·직렬 대기·셸 캐시 개선 | 완료 (v4.13.2) |
| 24 | 댓글 멘션 자동완성 · 답글을 진짜 멘션으로 · 댓글 수 정정 | 완료 (v4.14.0) |
| 25 | 안 쓰던 쓰기 경로 넷 — 시작일 · 만들 때 담당자·우선순위 · 하위 업무 추가 · 업무 아닌 글 | 완료 (v4.15.0) |

기능은 다 붙었다. 쓰기 경로는 실서버에서 확인했다 — 상태·마감일·우선순위·담당자 넷 다
REST 단일 필드 엔드포인트(`PATCH .../tasks/{taskId}/{field}`)로 테스트 업무 하나에서 바꾸고
되돌렸다. v4.15.0에서 붙인 **시작일 쓰기·담당자 실은 업무 생성·하위 업무 생성**은 같은 헬퍼를
...
| ~~`neutral` 톤에만 색 면이 없다~~ (v4.12.1에 뒤집음) | 당시 근거는 "넉 장을 다 물들이면 물든 게 기본이 돼서 다시 아무 신호도 아니다"였다. 그런데 `neutral`이 실제로 놓이는 자리가 오늘·팀 화면의 **넷째 칸(방치된 업무) 둘뿐**이라, 물든 셋 옆의 흰 상자 하나는 "평상시"가 아니라 덜 그려진 칸으로 읽혔다. 지금은 무채색 면을 준다 — 넷 중 가장 옅다 |
...
## Phase 19 — 검색이 네 갈래를 찾고, 줄마다 다른 곳으로 데려간다 (완료 · v4.11.0)
...
## Phase 20 — 새 소식이 오면 OS 알림 (완료 · v4.12.0)
