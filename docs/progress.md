...
버전 4.17.1 · 2026-08-07 기준. 로드맵 정의는 [PRD.md](PRD.md) §11에 있다.

> **Phase 0~8의 기록은 그때의 사실이다.** v1.3.0에서 데이터 통로가 MCP → REST 한 갈래로
...
| 6 | 내 업무 화면 (`/tasks`) | 완료 (v1.1.0) |
| 7 | 구성원 화면 (`/members`) | 완료 (v1.1.0) |
| 8 | 하위 업무 들여쓰기 | 완료 (v1.1.0) |
| 9 | REST 전면 이관 — MCP·OAuth 폐기 | 완료 (v1.3.0) |
| 10 | 이관 뒤 화면 훑기 — 안 쓰던 값 노출 | 완료 (v1.3.0) |
| 11 | 답글 읽기 · 멘션 모달 · 날짜 표기 통일 | 완료 (v1.3.0) |
| 12 | 내 업무 카드에 프로젝트 정보 | 완료 (v1.4.0) |
| 13 | 접힌 카드는 프로젝트, 펼친 카드는 업무 | 완료 (v1.4.0) |
| 14 | 나의 일정 줄 펼치기 — 상세 조회 | 완료 (v1.4.0) |
| 15 | 업무 상세 모달에 관계·첨부 | 완료 (v1.4.0) |
| 16 | 상위·하위 업무를 모달 안에서 펼치기 | 완료 (v1.4.0) |
| 17 | 첨부 이미지 뷰어 · `내 업무` 멈춤 해결(접힌 카드는 표를 안 만든다) | 완료 (v1.4.0) |
| 18 | 멘션 모달과 헤더 알림을 업무 상세 모달로 | 완료 (v1.5.0 · v1.5.0) |
| 19 | 검색이 넷을 찾고, 줄마다 다른 곳으로 데려간다 | 완료 (v1.5.0) |
| 20 | 새 소식이 오면 OS 알림 — 폴링에 얹은 알림 | 완료 (v1.6.0) |
| 21 | 오늘 일정 띠 · 답글은 그 자리에 · 폰에서는 탭이 고르개 | 완료 (v1.6.0) |
| 22 | 로딩 골격을 지금 화면에 맞춰 현행화 | 완료 (v1.6.0) |
| 23 | 화면 대기 시간 실측 후 프로젝트 훑기·직렬 대기·셸 캐시 개선 | 완료 (v1.6.0) |
| 24 | 댓글 멘션 자동완성 · 답글을 진짜 멘션으로 · 댓글 수 정정 | 완료 (v1.7.0) |
| 25 | 안 쓰던 쓰기 경로 넷 — 시작일 · 만들 때 담당자·우선순위 · 하위 업무 추가 · 업무 아닌 글 | 완료 (v1.7.0) |

기능은 다 붙었다. 쓰기 경로는 실서버에서 확인했다 — 상태·마감일·우선순위·담당자 넷 다
REST 단일 필드 엔드포인트(`PATCH .../tasks/{taskId}/{field}`)로 테스트 업무 하나에서 바꾸고
되돌렸다. v1.7.0에서 붙인 **시작일 쓰기·담당자 실은 업무 생성·하위 업무 생성**은 같은 헬퍼를
쓰지만 실서버 쓰기로는 안 눌러 봤다 — 공용 프로젝트에 실데이터가 남아서다. 유닛 테스트로
보내는 본문만 확인했다.

Phase 2에서 빠져 있던 알림 읽음 처리는 v1.0.0에서 REST 경로로 붙었다 (Tier A2).
...
[src/app/(app)/page.tsx](<../src/app/(app)/page.tsx>) — 요약 4칸 · 오늘의 포커스 10건 ·
밀리는 업무 · 방치된 업무 · 나를 부른 사람들(접기).
...
v1.0.0에서 여기에 밝기 두 벌이 붙었다 (아래 [밝기 세 갈래](#밝기-세-갈래-v0150)). 그때도
컴포넌트는 그대로였다 — 같은 이유로.
...
| 부서 탭 | `motion/tabs` (Pill — v1.0.0까지 Segment) |
...
## Phase 5 — REST 확장 (완료 · v1.0.0)

### 조사 (v1.0.0) — REST로 무엇이 더 되는지

**출발점**: v1.0.0에서 개인 API 키를 필수로 받기 시작했으니(그게 원래 REST를 버린 이유였다)
...
| B3 | 나의 일정 | 계정 서랍(≥1024px)·앱바 시트(<1024px). 오늘부터 이레(v1.1.0), 줄에 색·반복·프로젝트 링크(v1.1.0). 캘린더는 MCP에 길이 없다 |
...
### 나의 일정 줄에 색·참석·반복·프로젝트 링크 (v1.1.0)
...
| <1024px는 서랍이 아니라 바텀시트 | 계정 팝오버 자체가 ≥1024px 전용이라(§7.3) 앱바에 달력 단추를 따로 놓는다. 시트인 이유는 소식 종과 같다(v1.1.0) — 오른쪽 320px 판은 390px 화면에서 스크림을 한 뼘만 남기고, 여는 단추도 손이 가장 안 닿는 구석이다 |
...
v1.0.0에서는 "알림 응답으로는 딥링크를 못 만든다"고 적고 화살표를 껐다. **틀렸다.** 못 만드는 건
...
| 카드 = 링크(`item.href`, 새 탭) + 누른 순간 콜백(`item.onSelect`) | `motion/notification-stack.tsx` (v1.0.0에서 삭제) |
...
| 붙은 것 | 어디 |
|---|---|
| 탭 전체 · 안 읽음 · 읽음 — beUI `Tabs` 재사용 (v1.0.0에서 `underline`) | [news-bell.tsx](../src/components/news-bell.tsx) |
...
**탭 모양은 v1.0.0에서 갈랐다** — 알림 레이어는 `underline`, 부서 전환은 `pill`. 좁은 레이어에
...
### 내 업무 화면 설계 (v1.0.0) — 조사·설계

"내 참여 프로젝트와 프로젝트별 담당 업무 전량"을 볼 화면이 필요하다는 요청. 설계를
[PRD.md](PRD.md) §6.5에 썼다. 구현은 아래 "Phase 6"이다 (v1.1.0). 여기 적힌 880건은
...
> 밝게의 검정 액센트는 v1.1.0에 파랑으로 바꿨다 — 아래 「단색 화면과 여백 등급 (v1.1.0)」
...
| ~~`neutral` 톤에만 색 면이 없다~~ (v1.6.0에 뒤집음) | 당시 근거는 "넉 장을 다 물들이면 물든 게 기본이 돼서 다시 아무 신호도 아니다"였다. 그런데 `neutral`이 실제로 놓이는 자리가 오늘·팀 화면의 **넷째 칸(방치된 업무) 둘뿐**이라, 물든 셋 옆의 흰 상자 하나는 "평상시"가 아니라 덜 그려진 칸으로 읽혔다. 지금은 무채색 면을 준다 — 넷 중 가장 옅다 |
...
**라임은 v1.0.0에서 사라졌다.** 이 절을 쓸 때는 밝게 쪽만 `#4D7C0F`(4.6:1)로 갈아탄
...
> **v1.1.0에서 이 레일을 걷어냈다** (`overflow-actions.tsx` 삭제). 상단 바가 없어지면서
...
**레일 토글은 beUI 원본 모양으로 되돌렸다.** v1.0.0의 이니셜 원판을 물렸다.
...
### 머리의 상태 배지 → 등록자 (v1.2.0)

| 무엇 | 값 | 왜 |
|---|---|---|
| 업무명 아래 상태 배지 | 지웠다 | v1.2.0에서 `상태` 줄의 지금 값을 `StatusPill`로 바꾼 뒤부터 같은 배지가 한 화면에 두 번 나왔다. 남긴 쪽은 `상태` 줄 — 거기가 바꾸는 자리다 |
| 그 줄에 등록자 이름 | `등록자 {이름}` (`text-muted-foreground`) | 등록자는 어디서도 바꿀 수 없는 값이라 고치는 다섯 줄과 섞일 이유가 없고, 같은 줄의 마감 D-DAY·포커스 순위와 성격이 같다 |
| 값 위 등록자 덩어리 · 첫 글자 원판 | 없앴다 | flow가 이 컬럼에 사진을 안 줘서 글자 하나를 위해 28px을 쓰고 있었다 (v1.2.0에서 붙였던 것) |
...
[등록자 칸과 등록자 덩어리 (v1.2.0)](#등록자-칸과-등록자-덩어리-v310)에 있다.

### 상세 모달 가운데만 스크롤 (v1.2.0)
...
멘션 상세 모달은 알림 목록 하나뿐이라 아직 화면을 넘기지 않아 그대로 뒀다 — v1.2.0에서
붙였다(열 몇 번 불린 업무는 목록만으로도 화면을 넘긴다).

### 모달 벤더 교체 — beUI morphing-modal (v1.2.0)
...
| `src/components/last-comment.tsx` | **삭제**. v1.2.0 이후 부르는 곳이 없다 |
...
## Phase 6 — 내 업무 화면 (완료 · v1.1.0)

v1.0.0 설계(위 "내 업무 화면 설계")를 그대로 구현했다. 파일 셋이 새로 생기고 넷이 바뀌었다.
...
| `src/components/project-task-filter.tsx` | 프로젝트 안 목록을 상태 칩으로 거른다. **URL이 아니라 `useState`**다 — 칩 한 번에 59회 훑기와 3MB 재전송을 하지 않는다 (PRD §6.5). **v1.2.0에 지웠다** — 표가 스스로 거른다 |
| `src/components/task-item.tsx` | 마감일이 없으면 D-DAY 배지를 안 그린다 (785/951이 그렇다). **v1.2.0에 `TaskTable`로 바뀌었다** |
...
| `src/app/(app)/(today)/loading.tsx` | 오늘 화면 2단 — KPI 4칸 → 표 넉 장 한 단 (일정 8칸이 4단이었는데 v1.1.0에 셸로 갔고, 8:4 2단은 v1.2.0에 접혔다) |
| `src/app/(app)/risk/loading.tsx` | 제목 → 부서 탭 → KPI 4칸 → 프로젝트 요약 카드 4장(순위 칸 있음) |
| `src/app/(app)/team/loading.tsx` | 제목 → 부서 탭 → KPI 4칸 → 팀원 카드 4장(2줄씩. 카드가 펼쳐진 채로 온다. v1.2.0에 3단 → 1단이 되면서 여섯 장에서 줄였다) |
...
## Phase 11 — 답글 읽기 · 멘션 모달 · 날짜 표기 (완료 · v1.3.0)
...
## Phase 12 — 내 업무 카드에 프로젝트 정보 (완료 · v1.4.0)
...
## Phase 13 — 접힌 카드는 프로젝트, 펼친 카드는 업무 (완료 · v1.4.0)
...
## Phase 14 — 일정 줄을 펼치면 속내가 나온다 (완료 · v1.4.0)
...
## Phase 15 — 업무 상세 모달에 관계·첨부 (완료 · v1.4.0)
...
상위·하위 이름은 v1.4.0까지 **flow로 보냈다**. v1.4.0에서 모달 안 펼치기로 바꿨다 (Phase 16).
...
| [task-thread.tsx](../src/components/task-thread.tsx) | 섹션 셋 + `TaskRow`(v1.4.0 `TaskLink`)·`FileRow`·`fmtSize` |
...
## Phase 16 — 상위·하위 업무를 모달 안에서 펼치기 (완료 · v1.4.0)

v1.4.0이 이름을 냈지만 누르면 flow로 나갔다. 모달을 열어 둔 채로 딸린 업무를 확인할 수 없다.
...
## Phase 17 — 첨부 이미지 뷰어 · `내 업무` 멈춤 (완료 · v1.4.0)
...
## Phase 18 — 멘션 모달과 헤더 알림을 상세 모달로 (완료 · v1.5.0 · v1.5.0)

### 멘션 모달이 업무 상세 모달과 같아졌다 (v1.5.0)
...
## Phase 19 — 검색이 네 갈래를 찾고, 줄마다 다른 곳으로 데려간다 (완료 · v1.5.0)

`⌘K`가 프로젝트와 글만 찾았다. 사람 이름을 넣으면 그 사람이 쓴 글만 나오고 **그 사람**은 안
나왔고, 일정은 아예 못 찾았다. api-spec §9의 네 갈래(`projects`·`posts`·`employees`·`events`)를
다 붙였다.

### 구성원은 flow에 검색어를 안 넘긴다

§9.3의 `searchWord`는 **공용 API 키로 전 직원을 훑는 손잡이**다. 요청에서 받은 문자열을 그대로
흘리면 아무나 남의 이름을 넣어 명단을 긁을 수 있다 (PRD §8.1). 그래서 이 갈래만 경로가 다르다:

| | 검색어를 어디로 | REST |
|---|---|---|
| 프로젝트 · 글 · 일정 | flow로 보낸다 | 갈래마다 1회 |
| 구성원 | **안 보낸다** — 캐시된 명단에서 고른다 (`pickMembers`) | 0회 (`ROSTER_TTL` 600초 캐시) |

어차피 13명이라 메모리에서 훑는 게 왕복보다 싸다. **검색 한 번에 REST는 3회**다.

이름·부서·직책만 본다. 이메일은 줄에 그리지만 검색 대상에서 뺐다 — 도메인 두 글자에 전원이
걸린다. 인덱스가 아니라 순회라 눈으로 맞다고 보기 어려워서 테스트 4건을 옆에 뒀다.

### 일정은 창을 요구하고, 나갈 링크가 없다

§9.4는 `startDateTime`·`endDateTime`이 필수다. **−90일 / +180일**로 잡았다 — 지난 분기 회고와
다음 반기 일정이 다 들어온다. 응답에 `colaboSrno`가 없어서 **flow로 넘어갈 주소를 만들 수
없다.** 그래서 일정 줄은 일부러 안 눌리는 정보 줄이다. 대신 언제인지를 그 줄에서 답한다
(종일 · 여러 날 · 시각 세 갈래).

### 줄마다 가는 곳이 다르다

검색은 경유지다. 네 갈래가 다 flow 새 탭으로 나가면 경유지가 아니라 출구다 — 갈래마다 답이
있는 데까지만 데려다준다.

| 갈래 | 누르면 | 왜 |
|---|---|---|
| 업무 · 글 | **이 화면에서 업무 상세 모달** | 상태·마감일을 거기서 바로 고친다 |
| 구성원 | `/members?dept=` — 그 사람의 부서 탭 | 사진·번호·복사 단추가 거기 있다 |
| 프로젝트 | flow (새 탭) | 프로젝트 화면이 우리에게 없다 |
| 일정 | 안 눌린다 | 나갈 링크 자체가 없다 |

**업무·글은 알림 목록과 처지가 같다** — 손에 든 건 `postId`인데 모달의 쓰기 줄은 전부
`taskSrno`를 요구한다 (BUG-005). Phase 18이 news-bell에 세운 장치(상태 넷 + `loadNewsTask` +
`MorphingModal` + 낙관 병합 `patched`)를 `useTaskModal` 훅으로 빼서 둘이 나눠 쓴다. 그대로
두면 55줄이 두 벌이 될 자리였다 — `toFlowTask`를 뺀 것과 같은 이유다. 업무가 아닌
글(공지·회의록)만 flow 링크로 되돌리는 규칙도 훅이 같이 들고 간다.

**구성원은 `/members`가 `?dept=`를 받게 해서 보낸다** (`/risk`·`/team`과 같은 이름). 전체 13명
격자에 떨어뜨리면 방금 고른 사람을 다시 눈으로 찾아야 한다. 탭을 오가는 것 자체는 여전히
URL에 안 남긴다 — 들어오는 문만 열었다. `Tabs`의 `defaultValue`는 처음 한 번만 읽히므로
`key`도 같이 바꾼다. 안 그러면 같은 화면에서 다른 부서 사람을 또 골랐을 때 탭이 안 움직인다.

### 업무·글은 최신순으로 세운다

`/user/search/posts`의 `orderType` 기본값이 `SCORE`(관련도)다. 받은 순서를 그대로 그렸더니
날짜가 뒤죽박죽으로 읽혔다. `LATEST`로 부르면 관련도를 통째로 버리는 거라 안 맞는 최신 글이
여섯 줄을 채운다. **고르는 건 flow, 줄 세우기는 우리다** — 관련도 상위 6건을 받아서 그 안에서
최신순이다. `YYYYMMDDHHmmss` 고정폭이라 문자열 비교가 곧 시간 비교다.

### 여섯 줄 아래의 `더 보기`

여섯은 네 갈래를 한 화면에 담으려고 좁힌 수다. 그런데 찾던 것이 글일 때는 그 여섯이 **상위
여섯**일 뿐이라 원하던 게 그 아래에 있을 수 있다. 업무·글에만 `더 보기`를 달았다 —
프로젝트·구성원은 이름으로 찾는 것이라 상위 넷에서 갈리고, 일정은 애초에 안 눌리는 줄이다.

§9.1은 `score`+`pageTargetId` 짝으로 다음 쪽을 준다. **안 쓴다.** 같은 검색어를 `size`만 키워
(6 → 30) 다시 부른다:

| | 페이지 토큰 | `size` 키우기 |
|---|---|---|
| 왕복 | 1회 | 1회 |
| 이어 붙이기 | 겹치는 줄 걸러 내는 코드가 필요 | 통째로 갈아 끼워서 없음 |
| 낭비 | 없음 | 앞 여섯 줄을 다시 받는다 |
| 계약 | 안 써 본 짝 파라미터 | `size`는 이미 쓰고 있다 |

30줄 한 번 더 받는 낭비를 사는 대신 상태 하나로 끝난다.

**단추가 나올지는 flow가 정한다.** 응답의 `hasNext`를 그대로 쓴다 — 우리가 센 줄 수로 짐작하면
정확히 6건인 검색에도 단추가 붙고, 눌러 봐야 같은 여섯 줄이 돌아온다.

받고 나면 갈래를 `업무 · 글`로 같이 좁힌다. 서른 줄이 그냥 늘어나면 구성원·일정이 스크롤
저 아래로 묻히는데, 되돌리는 건 `전체` 칩 한 번이다. 검색어를 고치면 더 받아 둔 것도 같이
버린다 — 앞 검색어의 서른 줄이다.

`더 보기`는 한 단계뿐이다. 서른에서 또 붙이면 다음은 백이고, 백 줄을 팔레트에서 스크롤할
사람은 없다 — 거기까지 갔으면 검색어가 틀린 것이다.

### 갈래가 넷이 되면서 화면이 바뀐 것

- **한 갈래가 넘어져도 나머지는 보여 준다.** 전에는 하나 실패하면 통째로 오류였다. 이제 갈래마다
  따로 잡고, 못 가져온 갈래 이름만 결과 위에 한 줄로 밝힌다.
- **갈래 칩**으로 좁힌다. 결과가 두 갈래 이상일 때만 그린다 — 한 갈래뿐인데 칩을 그리면 누를
  이유가 없는 버튼이 다섯 개 생긴다. 0건인 칩은 비활성이다.
- **`↑` `↓` `↵`로 훑는다.** 팔레트 주석이 "목록이 길어지면 그때 붙인다"고 미뤄 뒀던 것인데,
  네 갈래가 그 시점이다. 렌더 순서를 상태로 또 적지 않고 `[data-row]`를 DOM에서 읽는다 —
  화면에 보이는 순서가 곧 이동 순서라 둘이 어긋날 수가 없다.

## Phase 20 — 새 소식이 오면 OS 알림 (완료 · v1.6.0)

종에 배지가 켜져도 그 화면을 보고 있지 않으면 모른다. 소식 판에 알림 스위치를 달았다.

### 진짜 Web Push가 아니다 — 폴링에 얹었다

flow는 알림을 밀어 주지 않는다. 웹훅도 구독도 없고 알림 목록 조회 하나뿐이라, 서버가 밀어 줄
원천 자체가 없다. 그래서 **이미 1분마다 돌고 있는 폴링(`/api/news`)에 얹었다** — 새로 당겨 온
목록에서 처음 보는 안 읽은 줄을 찾으면 그때 OS 알림을 띄운다.

| | 이 방식 | 진짜 Web Push |
|---|---|---|
| 서버 · 구독 저장소 · VAPID 키 | 없음 | 전부 필요 |
| 앱이 열려 있을 때 | 온다 (숨은 탭 · 다른 앱을 보는 중에도) | 온다 |
| 브라우저를 닫은 뒤 | **안 온다** | 온다 |

닫은 뒤까지 채우려면 구독 저장소 + 크론이 필요하고, 크론에는 세션이 없다. 지금 규칙은
"대상은 세션에서만 채운다"(PRD §8.1 · `rest.ts` · `/api/news`)인데 백그라운드 폴링은 `userId`를
저장해 두고 사용자 없이 부르는 구조라 그 선을 넘는다. **먼저 §8.1을 다시 잡아야 하는 일**이라
이번에는 안 건드렸다.

### 서비스 워커를 하나 넣었다 — 캐시 때문이 아니다

매니페스트는 "서비스 워커·오프라인 캐시는 넣지 않았다"고 적혀 있었다. 이유가 **캐시**였다:
이 앱은 열 때마다 flow에서 새로 읽어야 의미가 있어서 캐시된 화면은 틀린 화면이다.

`public/sw.js`는 그 이유에 걸리지 않는다 — `fetch` 핸들러가 아예 없어서 네트워크에 끼어들지
않는다. 워커가 필요한 건 다른 이유다: **안드로이드 크롬과 설치한 iOS PWA는 `new Notification()`을
거절하고 `registration.showNotification()`만 받는다.** 데스크톱만 노렸으면 워커 없이 됐겠지만
그러면 경로가 두 갈래가 된다.

등록도 **알림을 켠 사람에게만** 일어난다. 안 켠 사람 브라우저에 워커를 심어 둘 이유가 없다.

### 열자마자 쏟아지지 않게 — 기준선

가장 조심한 것이 이거다. 서버가 그려 준 첫 목록(`seed`)에 있던 알림은 "이미 본 것"으로 치고
`told`에 미리 담는다. 안 그러면 화면을 여는 순간 안 읽은 소식 전부가 알림이 된다.

첫 목록을 **못 가져왔으면**(`seed === null`) 아직 기준선이 없는 것이라, 첫 폴링 결과를 알리지
않고 기억만 한다 — 그것들은 새 소식이 아니라 원래 있던 것이다.

### 다섯 건이 와도 알림은 한 장

카드가 창 구석에 쌓이면 그걸 치우는 게 일이 된다. 여러 건이면 제목이 `새 소식 5건`,
본문이 그중 가장 새 것이다. `tag`는 가장 새 알림의 id다 — 탭을 두 개 열어 뒀으면 둘이 같은
묶음을 계산해서 같은 이름이 나오고, 브라우저가 하나로 겹쳐 준다. 이 조립만 순수 함수로
떼어 내 시험을 붙였다 (`newsAlert`, 6건).

### 숨어 있을 때도 당긴다 — 켠 사람만

폴링은 `document.hidden`이면 쉬고 있었다. 그런데 숨어 있을 때가 오히려 알림이 쓸모 있는
때다. 켠 사람만 계속 당긴다(분당 REST 2회). 안 켠 사람의 습관은 그대로다.

### 끄는 길과 막힌 길

브라우저 권한은 한 번 주면 프로그램으로 못 거둔다. 그래서 켬/끔을 `localStorage`에 따로
둔다 — 없으면 끄는 단추가 눌러도 아무 일 없는 죽은 단추가 된다. 상태는
`useSyncExternalStore`로 읽는다 (`use-narrow-screen.ts`와 같은 이유: 이펙트에서 `setState` 하면
첫 그림 뒤에 한 번 더 그리고, 린트도 막는다). 값을 바꾸는 게 우리뿐이라 구독자에게 직접 알린다.

브라우저가 차단해 둔 상태(`denied`)에서는 스위치를 흐리게 남기고 툴팁이 사유를 말한다.
`disabled`가 아니라 `aria-disabled`인 것은 **막힌 단추는 포인터 이벤트가 죽어서 툴팁이 안
뜨기 때문이다** — 하필 설명이 가장 필요한 상태에서 안 보이면 안 된다.

### 0건이어도 막대는 남는다

같이 고친 것. `Meter`가 칠할 세그먼트가 없으면 `null`을 돌려줬다. KPI 네 칸이 나란히 선
자리에서 한 칸만 막대 줄이 사라지니 그 카드만 키가 다르고 설명 줄이 위로 올라붙었다. 빈
트랙을 남긴다 — 없어진 자리는 아무 말도 안 하지만 빈 막대는 "0"이라는 말이다. 낭독에서는
뺀다(`aria-hidden`): 바로 위 숫자 줄이 이미 0을 말한다. 쓰는 자리가 여덟 곳인데 전부
"있는 것의 구성비"라 빈 트랙이 어색한 데가 없다.
...
**v1.0.0은 실화면으로 봤다.** 로그인 세션이 여전히 없어서, 셸에 목업 소식 6건을 넣은 임시
...
**v1.0.0도 같은 방식으로 봤다** (임시 `/login/preview` + 목업 6건, 확인 후 삭제). 확인한 것:
...
→ **앞의 것은 v1.0.0에서 "안 열린다"로 판명됐다** ([BUG-024](bug-report.md#bug-024)). 형식이
맞는 것과 로그인 리다이렉트가 대상을 들고 가는 것은 별개였다.

**v1.0.0·v1.0.0도 임시 `/login/preview` + 목업으로 봤다** (확인 후 삭제). v0.17에서 확인한 것:
