...
버전 1.2.1 · 2026-07-31 기준. 로드맵 정의는 [PRD.md](PRD.md) §11에 있다.
...
| 8 | 하위 업무 들여쓰기 | 착수 대기 (PRD §13 D1 · BUG-034) |
...
## Phase 8 — 하위 업무 들여쓰기 (착수 대기)

[PRD.md](PRD.md) §13 D1 · [BUG-034](bug-report.md#bug-034). `tasks/filter` 응답 최상위에
`upTaskId`(`-1`=최상위, 채움률 226/226)와 `subTaskCount`가 이미 와 있는데 우리가 안 읽고 있었다.
§6.5 한계 ①의 "`mode=TREE`는 미실측"은 틀렸다 — `TREE`·`tree`·`FLAT` 셋 다 같은 응답이다.
