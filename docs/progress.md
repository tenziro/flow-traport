...
오른쪽에서 서랍이 들어온다. 오늘 화면의 카드는 그대로 둔다 — 서랍은 어디서나 열리는 사본이지
대체가 아니다.
...
| 데이터는 `loadTodayEvents()` + React `cache()` | 셸과 오늘 화면이 같은 하루를 부른다. 인자를 안 받는 것도 캐시 키 때문이다(`Date.now()`가 갈리면 두 번 나간다). 실측: `/` 한 요청에 REST 왕복 **1회** |
| <1024px에는 없다 | 계정 팝오버 자체가 ≥1024px 전용이다(§7.3). 좁은 화면은 하단 탭으로 오늘 화면에 바로 간다 |
