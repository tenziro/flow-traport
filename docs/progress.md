...
`CopyButton`에 `className` 선택 인자를 더했다(`ml-auto shrink-0`을 주려고). 기존 두 호출부는
안 넘기므로 그대로다.

실측(브라우저, 재측정): 1280px 3열 · 카드 높이 138px(한마디 있으면 175px) · 375px 1열, 두 폭
모두 가로 넘침 없음, 사진 10/10 `200`, 다크 모드 정상, 하단 탭 5칸 75px 그대로. **손해 하나**:
375px 세로 스크롤이 1,500px → 1,900px로 약 25% 늘었다.
