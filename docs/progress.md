...
실루엣으로 두지 않은 것과 같은 함정이다. 값은 격자 아래끝이 들쭉날쭉해지는 것이고(13명 중 한마디
2명이라 5행 중 2행), 카드가 내용에서 끝나는 건 결함으로 읽히지 않는다. 부서도 카드로 싸지 않는다 —
사람이 카드인데 부서까지 카드면 테두리가 두 겹이다.
...
실측(브라우저, 재측정): 1280px 3열 · 카드 높이 136px(한마디 있으면 173px) · 375px 1열, 두 폭
