# 헤더 오버플로 레일 · 타이틀 스윕 설계 (폐기)

**날짜** 2026-07-29
**버전** 0.21.0 → 0.22.0 (추가) · **v1.0.0에서 폐기**

> **이 설계는 더 이상 코드에 없다.** v1.0.0이 상단 바 2행을 걷고 좌측 레일을 세우면서
> `motion/overflow-actions.tsx`를 지웠다 — 밝기·계정·로그아웃을 한 줄에 묶어야 했던 이유,
> 즉 "헤더 오른쪽에 여섯 덩어리"라는 전제 자체가 없어졌다. 밝기는 라디오 판으로,
> 계정은 레일 발의 팝오버로 갔다 (PRD §7.3).
>
> 남는 것은 타이틀 스윕(`ChromaticTextReveal`)과 BUG-029의 교훈이다. 아래는 기록이다.
...
> **v0.23.0에서 이 절을 되돌렸다.** 이니셜 원판은 "누르면 뭔가 나온다"는 말을 못 한다 — 아바타는 신분 표시이고 펼침 어포던스가 아니다. beUI 원본 그대로 늘 `bg-primary`에 `⋯`(점 셋, `Filled`) ↔ `✕`를 blur로 갈아치운다. 접힌 트랙에서 색을 가진 것이 이 원판뿐이라 펼칠 데가 있다는 걸 이것만 보고 안다. 아래 §닫는 길 셋 / §접근성 / §측정의 "이니셜"은 모두 이 원판을 가리킨다.
>
> **같은 배포에서 이름·부서 두 줄도 뺐다** (§이름·부서만 배경이 없다 절이 함께 무효가 된다). 로그인은 한 계정뿐이라 화면에서 이름을 확인할 일이 없는데 두 줄 텍스트가 펼침 폭의 절반을 먹었다 — 실측 303px → 251px. 이름은 토글 `aria-label`에만 남는다. 펼침은 이제 밝기 · 로그아웃 둘이고, Tab 순서는 알림 → 밝기 → 로그아웃 → 토글이다.
>
> **모서리와 높이**: 트랙은 §트랙 모서리 절과 달리 원본 `rounded-full`로 되돌렸고 안쪽 항목·헤더 검색도 알약으로 맞췄다. 높이는 원본 치수(여백 `p-1.5` + 항목 36px = 50px)가 56px 헤더에서 너무 커서 여백을 `p-px`로, 항목을 32px로 줄여 **36px**로 뒀다. 검색도 같은 36px(`min-h-9`)이다.
