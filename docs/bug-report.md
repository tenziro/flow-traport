...
| ID | 대상 | 상태 |
|----|------|------|
| [BUG-001](#bug-001) | flow MCP `flow_list_alarms` | 없어짐 (v1.3.0 — MCP 폐기) |
...
| [BUG-007](#bug-007) | flow MCP `flow_list_projects` | 없어짐 (v1.3.0 — MCP 폐기) |
| [BUG-008](#bug-008) | `"use server"` 모듈의 상수 export 소실 | 해결 |
| [BUG-009](#bug-009) | 아코디언 본문 `-mx-5`로 UI 잘림 | 해결 |
| [BUG-010](#bug-010) | 헤더 블러가 눈에 안 보였다 | 해결 |
| [BUG-011](#bug-011) | `flow_collect_project_chain` 권한 오류 | 없어짐 (v1.3.0 — MCP 폐기) |
...
| [BUG-034](#bug-034) | 응답에 있는 `upTaskId`를 못 보고 "계층을 못 그린다"로 닫았다 | 해결 (v1.1.0) |
| [BUG-035](#bug-035) | `systemCode`가 채워졌으면 시스템 댓글로 봤다 — 사람 댓글 38%가 사라졌다 | 해결 (v1.1.0) |
| [BUG-036](#bug-036) | `revalidatePath`에 쿼리스트링을 넘겨 캐시가 안 지워졌다 | 해결 (v1.1.0) |
| [BUG-037](#bug-037) | 모달에서 바꾼 값이 업무 줄에 6.5초 뒤에야 닿았다 | 해결 (v1.1.0) |
| [BUG-038](#bug-038) | 우선순위·담당자를 한 번 못 가져오면 새로고침까지 잠겼다 | 해결 (v1.1.0) |
| [BUG-039](#bug-039) | 내용이 긴 모달이 위아래로 잘려 끝까지 못 올라갔다 | 해결 (v1.2.0) |
| [BUG-040](#bug-040) | 분당 120회를 넘겨 프로젝트 59개 중 26개가 조용히 빠졌다 | 해결 (v1.3.0) |
| [BUG-041](#bug-041) | REST에는 답글이 없다 — 답글이 최상위 댓글로 붙었다 | 해결 (v1.3.0) |
| [BUG-042](#bug-042) | 쿼리를 붙이고도 응답의 새 필드를 안 봐서 답글이 통째로 빠졌다 | 해결 (v1.3.0) |
| [BUG-043](#bug-043) | 나를 부른 댓글 강조를 알림으로 맞춰 옛 멘션이 빠지고 내 말이 걸렸다 | 해결 (v1.3.0) |
| [BUG-044](#bug-044) | `내 업무`가 하이드레이션 전까지 `opacity:0`으로 통째로 비어 보였다 | 해결 (v1.4.0) |
| [BUG-045](#bug-045) | 화면은 뜨는데 브라우저가 느려지다 멈췄다 | 해결 (v1.4.0) |
| [BUG-046](#bug-046) | 상태 고르기에 지금 값(`대기`)이 없었다 — 한 상태를 두 이름으로 불렀다 | 해결 (v1.7.0) |
| [BUG-047](#bug-047) | 답할 상대를 자동완성으로 고르면 멘션이 두 번 저장됐다 | 해결 (v1.7.0) |
| [BUG-048](#bug-048) | 업무인 알림 줄이 `업무가 아닌 글`로 열리거나 flow로 나갔다 | 해결 (v1.8.0) |
| [BUG-049](#bug-049) | 첨부 이미지를 눌러도 뷰어가 안 떴다 — 이펙트 정리가 `close()`를 불렀다 | 해결 (v1.10.0) |
| [BUG-050](#bug-050) | 댓글에 붙은 파일이 아예 안 보였다 (+ 첨부 모양이 자리마다 달랐다) | 해결 (v1.10.0) |
| [BUG-051](#bug-051) | 크로미움에서 `내 업무` 화면이 통째로 안 그려졌다 — 닫힌 모달 42겹이 합성기를 무너뜨렸다 | 해결 (v1.12.0) |
...
**재측정 (2026-07-31)**: "모든 입력에서"는 과했다. 프로젝트 ID 3개로 다시 불러 보니 **1개는 열렸고**
(최상위 3건 · 그 아래 7건 · 15건까지 정상), 나머지 둘은 같은 오류였다. 거부된 쪽도
`flow_get_my_profile`로 확인한 내 계정이 **참여자 목록에 있는** 프로젝트다 — 참여가 아니라
소유/관리 권한을 보는 것으로 보인다. 같은 계열 `flow_collect_dept_chain`은 30일 창에서 부서 활동으로
**프로젝트 1개**만 봤다(우리 팀은 59개에 붙어 있다). chain 계열에 팀 화면이나 주간보고를 얹을 수 없다
([PRD §13 D2](PRD.md#d2--도구는-있는데-권한이-막는다)).
...
**멘션 알림을 첫 100건만 받는다 — `hasNext`가 이미 켜져 있다** — 2026-07-28 확인, **해결** (v1.0.0)
...
**처리 (v1.0.0)**: `listAlarms(filters, { days })`가 `lastCursor`로 커서 루프를 돈다. 종료
...
생김새는 그대로다. → **v1.0.0에서 이 컴포넌트는 지웠다** (PRD §6.1.5 구조). 아래 교훈은 다른
...
- 스택은 `aria-label`을 붙인 `<button>` 하나다. **라벨이 이기니 안쪽 카드 글자가 스크린
  ...
  → **v1.0.0에서 `sr-only` 목록은 지웠다.** 카드를 링크로 바꾸면서 바깥 `<button>`을 벗겼고
...
v1.0.0에서 헤더 알림 종을 붙이면서 소식 줄에 링크를 안 달았다. 근거로 적은 문장이 틀렸다:
...
`TaskNews.url`로 내려보낸다 (v1.0.0). 네 곳의 문장은 고쳤다.
...
확인했다. → **v1.0.0에서 스택 자체를 물리면서**(PRD §6.1.5 구조) 목록이 평범한 `<ul>`이
...
- `whileTap={{ scale: reduce ? 1 : 0.96 }}` — prop은 늘 있고 배율만 1이다
  ...
  `motion/overflow-actions.tsx`는 v1.1.0에서 지워졌다 — 상단 바가 좌측 레일로 바뀌면서
...
**응답에 있는 `upTaskId`를 못 보고 "하위 업무 계층을 못 그린다"로 닫았다** — 2026-07-31, 해결 (v1.1.0)

v1.1.0에서 `/tasks`를 만들 때 계층을 포기하고 PRD §6.5 한계 ①에 이렇게 적었다: "`upTaskName`이 전부
빈 문자열이고 … 평평하게 나열한다 (`mode=TREE`는 미실측)". 앞은 맞고 **뒤가 틀렸다.**

`mode`를 실측하니 `TREE`·`tree`·`FLAT` 셋 다 같은 25건을 돌려준다 — 아무 효과가 없다. 그런데 같은
응답의 **최상위**에 안 읽던 필드가 둘 있었다.

| 필드           | 값                      | 실측 (담당 필터, 12개 프로젝트 226건)              |
| -------------- | ----------------------- | -------------------------------------------------- |
| `upTaskId`     | `-1` 또는 부모 `taskId` | 채움률 **226/226**. `-1`(최상위) **35건**          |
| `subTaskCount` | 직속 자식 수            | 필터 없는 응답에서 실제 자식 수와 일치 (7→7, 15→15) |

`flow_collect_project_chain`이 열어 준 프로젝트 하나(BUG-011 재측정)의 트리와 `upTaskId`로 접은
결과가 일치해서 뜻을 확정했다.

**왜 놓쳤나**: 계층 필드를 `columns` 배열 안에서만 찾았다. 업무 값은 대부분 `columns`에
`defaultColumnType` 키로 들어오니까(§6.5) `upTaskName`이 비어 있는 것을 보고 "계층 정보가 없다"로
결론냈다 — 정작 두 필드는 `columns` 밖 최상위에 있었다. **한 군데가 비면 응답 전체 키를 다시 찍는다.**

**처리**: PRD §6.5 한계 ①·§13 D1·Phase 8을 고치고 **v1.1.0에서 구현했다**. 부모까지 내 목록에 있는
건이 191건 중 26건뿐이라, 들여쓰기는 그 26건에만 붙이고 나머지는 평평하게 둔다. 없는 부모를 받으려면
건당 조회 165회가 필요하고 분당 120회 예산에 걸린다. 실데이터로 확인: `[HD현대그룹] 출장관리`에서
상위 업무 둘 아래로 7건이 붙고 건수는 그대로다(15+22=37).

---

## BUG-035

**`systemCode`가 채워졌으면 시스템 댓글로 봤다 — 사람 댓글 38%가 사라졌다** — 2026-07-31, 해결 (v1.1.0)

"상태가 피드백이고 마지막 댓글이 내 것이면 포커스에서 뺀다"를 만들면서 `lastHumanComment`를 실데이터에
걸었다. `Q022 매핑코드`는 댓글이 4건인데 `null`이 나왔다 — 사람 댓글이 하나도 없다는 뜻이다. flow 화면에는
사람이 쓴 말이 그대로 있었다.

원인은 판별식이었다. §13.1을 처음 쓸 때 실측 14건 중 10건이 변경 로그였고 그 10건 모두 `systemCode`가
채워져 있어서 **`systemCode`가 truthy면 변경 로그**로 닫았다. 게시글 12건·댓글 148건을 다시 찍으니:

| `systemCode` | 건수 | 정체 |
|---|---|---|
| (없음) | 78 | 사람 댓글 |
| `S14` · `S13` · `S20` | 56 | **사람 댓글** (본문이 사람 말이다) |
| `S41^^'김동석'@$%` · `S45^^0^^1` · `S47^^2026-07-29` · `S83^^이성우` | 14 | 변경 로그 |

값 없는 맨 코드는 댓글에 붙는 꼬리표(수정 표시·멘션 종류 등)일 뿐 본문은 사람이 쓴 말이다. 56건이면
채워진 것의 80%, 전체 사람 댓글의 38%다 — 업무 줄의 "마지막 댓글"이 그만큼 비어 있었다.

**처리**: `isChangeLog(systemCode) = systemCode?.includes("^^")` (`src/lib/flow/rest.ts`). 변경 로그는
늘 `코드^^값` 꼴이고 항목은 `@$%`로 이어진다 — 구분자가 있으면 로그, 없으면 사람 말이다.
`lastHumanComment`와 스레드 표시(`loadThread`)가 같은 함수를 쓰고, §13.1 표와 경고문도 고쳤다.

**교훈**: **"채워져 있으면 X다"는 관측 표본이 한쪽으로 몰렸을 때 나오는 결론이다.** 14건은 코드의
*종류*를 세기엔 너무 적었다. 필드의 뜻을 정할 때는 값의 유무가 아니라 **값의 모양**을 본다.

---

## BUG-036

**`revalidatePath`에 쿼리스트링을 넘겨 캐시가 안 지워졌다** — 2026-07-31, 해결 (v1.1.0)

"모달에서 상태를 바꿨는데 바닥 페이지가 안 바뀐다"를 쫓다 발견했다. 리스크·팀 화면은 업무 줄에
`path={`/risk?dept=${encodeURIComponent(dept)}`}`를 넘겼고, 서버 액션은 그 값을 그대로
`revalidatePath(path)`에 넣었다.

`revalidatePath(p)`는 내부적으로 `revalidateTag("_N_T_" + p)`다 — **경로를 문자열 태그로 쓴다.** 렌더할
때 붙는 태그는 쿼리스트링 없는 `/risk`라서, `/risk?dept=…`는 **어느 태그와도 안 맞는다.** 지워지는 게
없다. Next는 없는 태그를 지워도 오류를 내지 않아서 조용히 아무 일도 안 일어난다.

지금 이 두 화면은 ttl 붙은 fetch가 없어서(전부 `no-store`) 눈에 보이는 증상은 없었다. 현재 라우트는
액션 응답에 실려 다시 그려지기 때문이다. **캐시를 하나만 붙이는 순간 조용히 낡은 값이 남는다.**

**처리**: 두 화면 모두 모듈 상수 `const PATH = '/risk'` / `const path = '/team'`로 바꿨다. 부서는 URL에
그대로 있어서 다시 그려도 보고 있던 부서가 유지된다 — 쿼리를 태그에 넣을 이유가 없었다. 쓰지 않게 된
`dept` prop은 `RollupCard`·`TaskRow`에서 걷었다.

**교훈**: **`revalidatePath`에 넘기는 값은 라우트지 URL이 아니다.** 검색 파라미터·해시가 붙으면 무효
호출이 되고, 실패가 조용해서 캐시를 붙일 때까지 안 드러난다.

---

## BUG-037

**모달에서 바꾼 값이 업무 줄에 6.5초 뒤에야 닿았다** — 2026-07-31, 해결 (v1.1.0)

신고는 "모달에서 값을 바꾸면 수동으로 새로고침하기 전까지 바닥 페이지가 안 바뀐다"였다. BUG-036에서
무효 `revalidatePath` 하나를 걷었지만 증상이 남아서, 사용자 승인을 받아 **실서버 쓰기로 재현**했다.
테스트 업무(`LGI-REQ-기타-일반-테스트-001`) 하나의 마감일을 실제 크롬에서 바꾸고, `MutationObserver`로
DOM이 바뀌는 시각을 밀리초로 찍었다.

```
1064ms  저장 중…                       ← 액션 시작
1529ms  모달[…마감일을 2026-07-15로 바꿨어요.]  ← 액션 반환. 모달 자기 값은 즉시 바뀐다
8069ms  행[… D+16 … 마감일 2026-07-15]  ← 바닥 업무 줄. 6.5초 뒤
```

**원인은 flow도 캐시도 아니었다.** `revalidatePath`는 정상 작동한다 — 다만 그 결과가 **페이지 하나를
통째로 다시 그린 RSC 페이로드**로 액션 응답에 실려 오고, React는 `useActionState`의 커밋을 그 페이로드가
적용될 때까지 트랜지션 안에 붙잡아 둔다. 내 업무 화면은 프로젝트마다 REST를 부르므로 그 재렌더가 실측
6.5초다. **성공 문구조차 그때 함께 나타났다** — 액션이 1.5초에 끝났는데도.

같은 리듀서 안의 평범한 `setState`(`setSaved`·`setEditing`)는 즉시 커밋된다. 이 비대칭이 처리의 열쇠다.

**즉, "안 바뀐다"는 사실은 "6.5초 뒤에 바뀐다"였다.** 사용자는 그 전에 모달을 닫고 옛 값을 보고
새로고침한다 — 증상 신고가 정확했고 원인 추정만 틀렸다.

**처리**: 업무 줄이 저장 성공을 직접 받아 자기 값을 먼저 갈아 끼운다.

- `TaskItem`을 클라이언트 컴포넌트로 올렸다. 자식 넷(`StatusPill`·`Meter`·`FlowLink`·`LastComment`)은
  서버 전용 코드가 없어서 경계를 올려도 부르는 데이터가 늘지 않는다.
- `TaskActions` → `Panels` → `EditDialog` → `useSave`로 `onSaved` 콜백 하나를 뚫었다. `useSave`는 액션이
  `ok`일 때 이미 `setSaved`·`setEditing`을 부르는데, 거기서 저장한 값을 그대로 위로 올린다.
- 넘기는 건 **업무 줄에 보이는 두 값(상태·마감일)뿐**이다. 우선순위·담당자는 줄에 없다.
- 낙관값에 **얹은 시점의 서버 값(`base = status|endDate`)을 같이 담는다.** 렌더는
  `live?.base === base ? live : task` 한 줄이라, 서버가 다른 값을 주면(내 저장이 반영됐든 남이 바꿨든)
  낙관값을 저절로 버린다 — 이펙트도 타이머도 없다.
- 마감일이 바뀌면 남은 일수를 `diffDays(Date.now(), parseFlowDeadline(endDate))`로 다시 센다. `Date.now()`가
  렌더가 아니라 `onSaved` 핸들러 안에 있는 이유는 `react-hooks/purity`다 — 렌더에서 부르면 lint가 막는다.

같은 측정 재실행: **1294ms**(액션 반환과 같은 시점)에 행이 `D+16` · `2026-07-15`로 바뀌었다. 서버 재렌더는
7828ms에 도착해 낙관값을 조용히 대체했다. 되돌리기도 2041ms에 `D+92` · `2026-04-30`으로 돌아왔다.
**6.5초 → 0.2초.** 상태(MCP)는 같은 `useSave` 경로라 함께 적용되지만, 브라우저 검증은 마감일(REST)로 했다
— 스크래치패드 세션의 액세스 토큰이 더미여서 MCP는 401이다.

**교훈 1**: **`revalidatePath`가 되는데도 화면이 늦으면 캐시가 아니라 재렌더 시간을 재라.** 서버 액션의
응답에는 그 경로의 전체 렌더가 실린다. 페이지가 느리면 액션 결과 커밋도 그만큼 늦는다 — 로직은 다 맞고
사용자만 못 기다린다.

**교훈 2**: 낙관 업데이트를 되돌리는 데 이펙트가 필요 없다. **얹은 시점의 서버 값을 같이 들고 있으면**
비교 한 줄이 스위치가 된다. `useEffect`로 서버 값 변화를 감시하는 코드는 이 문제에서 전부 불필요하다.

**교훈 3**: 실서버 쓰기로만 잡히는 버그가 있다. 값을 되돌릴 수 있는 테스트 업무 하나에 한정하고, 쓰기
전후 상태를 매번 확인하고, 오클릭으로 틀린 값이 들어가면 즉시 복구한다(실제로 한 번 밟았다 — react-day-picker의
달 격자 첫 줄에는 **이전 달 날짜가 섞여 있어서** `4월 격자의 첫 "30"`은 3월 30일이었다).

---

## BUG-038

**우선순위·담당자를 한 번 못 가져오면 새로고침까지 잠겼다** — 2026-07-31, 해결 (v1.1.0)

BUG-037을 고치는 중에 모달의 우선순위·담당자가 `지금 값을 못 가져왔어요`로 뜬 것을 한 번 봤다.
쫓아 보니 **flow도 조회도 멀쩡했고**, 클라이언트가 실패를 다루는 방식에 결함이 둘 있었다.

먼저 조회 자체를 실측했다. `getTaskFields`를 같은 인자로 직접 3회, 브라우저에서 새로고침을 끼워 3회
— **6/6 성공, 채워지는 데 257ms**다. `searchWord`도 정상이다(제목 전문 → 2건, 대상 포함).

```
1) searchWord=제목 전문   {count:2,   hit:true}
2) searchWord 없음        {count:100, hit:false}   ← 전량 훑기로는 첫 장에 없다. 검색이 필요한 이유
3) searchWord=테스트-001  {count:2,   hit:true}
```

**즉 원인은 조회가 아니다.** 원래 관찰한 그 한 번의 실패가 왜 났는지는 재현하지 못했다 — 그건
모른다고 적어 둔다. 다만 그 뒤에 벌어지는 일은 전부 재현됐다.
...
function onOpenChange(open: boolean) {
  ...
  setAsked(true);          // ← 성공·실패를 안 가린다
  startLoad(async () => { … });
}
```

`asked`는 "모달을 열 때 한 번만 부른다"를 지키는 가드다(업무 한 줄에 REST 한 번이라 필요하다).
그런데 **실패해도 참으로 남아서**, 모달을 닫고 다시 열어도 다시 묻지 않는다. 그 화면이 살아 있는
동안 계속 `못 가져왔어요`고 **탈출구가 새로고침뿐**이다 — 사용자가 실제로 하는 행동(다시 열어 보기)이
아무 효과가 없다.

### 결함 2 — 조회가 끊기면 화면이 통째로 죽는다

액션 요청 자체를 끊고(POST abort) 재 보니, 모달은 아예 열리지 않고 화면이 오류 경계로 넘어갔다:

```
(차단 중) 모달: (모달 없음)   POST 2회 끊김 ← Next가 한 번 재시도한다
(차단 중) 화면: … 화면을 다시 불러와 주세요 / flow에서 정보를 받아오다 멈췄어요.
```

`loadTaskFields`의 `try/catch`는 **서버 안**에서 난 오류만 싼다. 네트워크 끊김·서버 재시작·세션
만료처럼 **요청 자체가 실패하면** `await`가 거부하고, 그 거부가 `startTransition` 안에서 처리되지
않은 채 오류 경계까지 올라간다. 정작 그 경우를 위해 만들어 둔 `못 가져왔어요` 문구는 쓰이지도 못한다.
**곁가지 조회 하나가 화면을 죽일 자격을 갖고 있었다.**

**처리** — `EditDialog`의 로드 한 곳에서 둘 다 닫았다.
...
조회 중에는 `asked`가 참이라 다시 열어도 두 번 부르지 않는다 — 가드는 살아 있고 실패만 안 잠긴다.

검증(브라우저, 쓰기 없음):

| | 처리 전 | 처리 후 |
|---|---|---|
| 차단 중 화면 | 오류 경계 — 내 업무 화면이 사라짐 | 정상. 모달에 `못 가져왔어요` |
| 닫고 다시 열기 | `못 가져왔어요` 그대로 | **POST 1회 → 300ms 안에 값** |
| 평상시 | 257ms | 257ms (그대로) |

**교훈 1**: **"한 번만 부른다" 가드는 성공만 기억해야 한다.** 실패까지 기억하면 가드가 잠금이 된다.
`setAsked(true)`를 호출 전에 두는 건 중복 호출을 막으려는 것이므로 맞다 — 실패했을 때 되돌리는 짝이
빠져 있었을 뿐이다.

**교훈 2**: **서버 액션의 `try/catch`는 전송 실패를 못 잡는다.** 액션이 `{ok:false}`를 돌려주는 건
서버까지 갔다 온 경우뿐이다. 화면이 없어도 되는 곁가지 조회라면 호출부에 `.catch`가 있어야 하고,
없으면 오류 경계가 그 조회 하나 때문에 화면 전체를 가져간다.
...
**내용이 긴 모달이 위아래로 잘려 끝까지 못 올라갔다** — 2026-08-03, 해결 (v1.2.0)

신고는 "모달 내용이 길어질 경우 내용 짤림"이었다. 본문과 댓글이 모달로 들어온 뒤(v1.2.0)
...
알려진 상호작용). 짧은 내용에서는 안 보이는 결함이라 v1.2.0까지 드러나지 않았다.
...
**이탈 5**로 적었다 (v1.2.0에는 `center-morph-modal.tsx`의 이탈 #4였다 — v1.2.0에서 벤더를
갈면서 이 칸을 그대로 옮겼다).
...
**증상**: MCP를 걷고 REST로 갈아 끼운 직후, 오늘 화면을 연달아 열자 프로젝트 **59개 중 26개**가
비었다. 화면에는 아무 표시도 없이 임박·밀림·방치 숫자만 조용히 줄었다.

**원인**: 분당 120회 상한이다. 화면 한 번이 81회고(api-spec §12), 업무 조회 캐시가 **60초**라
캐시가 만료된 뒤 새로 고치면 두 번째 훑기가 상한을 넘는다.

```
429  RATE_LIMIT_EXCEEDED_ERROR
     요청이 너무 많습니다. (분당 최대 요청가능 횟수: 120)
```

`collectTasks`는 프로젝트별 조회를 `.catch(() => null)`로 감싸 한 프로젝트가 죽어도 화면이
서게 해 뒀는데, **그 실패를 아무도 안 읽고 버렸다.** 방어가 곧 은폐가 됐다.

**처리** — 두 갈래다.

1. **덜 부른다.** 업무 조회 데이터 캐시를 60초 → **300초**(`TASK_TTL`). 오늘 화면과 내 업무
   화면이 **같은 URL**을 부르게 둔 게 이 때문이다 — 두 화면이 캐시 한 칸을 나눠 쓴다.
   멘션 조회 창도 30일 → 7일, 줄 수 20 → 12로 줄였다 (알림 페이지가 절반).
2. **못 읽었으면 말한다.** `collectTasks`가 실패한 프로젝트 **이름**을 `failed`로 내고,
   `loadToday`가 그걸 `TodayData`에 실어 화면 아래에 적는다.
   ([queries.ts](../src/lib/flow/queries.ts), [(today)/page.tsx](<../src/app/(app)/(today)/page.tsx>))

```
"OOO, XXX는 지금 조회가 막혀서 위 숫자에서 빠져 있어요. 잠시 뒤에 새로 고쳐 보세요."
```

**남는 것**: 상한 자체는 안 없어진다 (PRD R8). 캐시가 다 만료된 상태에서 여러 사람이 동시에
들어오면 또 밟는다. 그때도 숫자가 조용히 줄지는 않는다는 것이 이 처리의 전부다.

**교훈**: `.catch(() => null)`은 실패를 **삼키는** 코드다. 삼킨 것을 화면까지 들고 갈 경로가
없으면, 그건 견고함이 아니라 조용한 오답이다.

> **반쪽이었다 (v1.3.0)**. 위 2번을 오늘·내 업무 두 화면에만 붙였고, **팀·리스크는 `loadTeam`이
> `collectTasks`가 준 `truncated`/`failed`를 그대로 버리고 있었다.** 하필 부서 전체를 훑어
> 429가 제일 잘 나는 자리다. 두 화면도 `TeamData`로 값을 들고 나와 적는다 — 리스크는
> `loadRisk`가 `...team`을 펼치므로 같이 따라온다. 네 화면의 문구는 `CollectNotice` 한 곳으로
> 모았다 ([collect-notice.tsx](../src/components/collect-notice.tsx)).
>
> 같은 교훈의 두 번째 얼굴이다: 실패를 들고 갈 경로를 **한 화면에만** 놓으면, 나머지 화면에서는
> 고치기 전과 똑같이 조용하다.

**상태**: 해결 (v1.3.0) · 팀·리스크 화면까지 확장 (v1.3.0)

---

## BUG-041

**증상**: v1.2.0에 붙인 답글이 REST로는 안 된다. 답글을 달면 스레드가 아니라 **최상위 댓글**로
붙는다.

**원인**: `POST /user/comments/{postId}`의 Body는 **`{ "contents": string }` 하나**다 (실측
2026-08-04). 부모 댓글을 가리킬 필드가 스펙에도 응답에도 없다. MCP `flow_create_comment`에는
있던 것이라, 이관하면서 대응물이 없다는 걸 늦게 봤다.

함정이 하나 더 있다. 필드 이름이 **`contents`**다 — `content`로 보내면 400도 아니고
**빈 댓글이 등록된다.**

**처리**: 답글 버튼을 없애지 않고 의미를 바꿨다. 누르면 댓글 칸에 **`@이름 `**을 미리 채운
최상위 댓글이 된다. flow 화면에서 읽을 때 누구에게 하는 말인지가 남고, 멘션 알림도 간다.
([api-spec.md](api-spec.md) §13.2)

**상태**: 해결 (v1.3.0) — 스레드 자체는 flow REST가 열어 주기 전까지 못 만든다.

> **읽기는 열려 있었다 (v1.3.0)**: [BUG-042](#bug-042). 여기서 "못 만든다"고 적은 건
> **쓰기**만 해당한다. 남이 flow에서 단 답글을 **읽어 오는** 경로는 처음부터 있었다.

---

## BUG-042

**증상**: 댓글 목록에 답글이 통째로 빠졌다. 실측 게시글 82396719는 댓글 4건 중 **3건에 답글**이
달려 있었는데 화면에는 최상위 4줄만 섰다. 마지막 말이 답글인 업무는 "마지막 댓글"도 틀렸다 —
답으로 대화가 끝났는데 화면은 그 앞의 물음을 마지막 말로 보여 준다.

**원인**: 조사 기록이 틀렸다. 앞선 세션에 `GET /user/comments/{postId}?replyYn=Y`를
**실제로 불렀고 200을 받았는데**, 응답의 **최상위 키만** 앞뒤로 비교하고 "쿼리는 받지만 답글은
안 온다"고 적었다. 답글은 최상위가 아니라 **댓글마다 붙는 새 필드**(`replies[]`,
`replyHasNext`)로 왔다. 없는 걸 못 본 게 아니라, **있는 걸 안 본** 것이다.

그 기록이 api-spec에 "답글은 안 온다"로 남고, PRD가 그걸 근거로 "목록을 한 층으로 편다"는
결정을 적고, 코드가 그 결정을 따랐다. 한 번의 관찰 실수가 세 문서를 거쳐 화면까지 갔다.

**처리**: 목록 조회에 `?replyYn=Y`를 붙이고(`listComments`), 댓글 뒤에 자기 답글을 시각순으로
잇는다(`toThread`). **추가 호출은 0회다** — 댓글 10건을 넘는 댓글만 `replyHasNext`로 표시되고,
그때만 `GET …/replies/{commentId}`를 한 번 더 부른다(`fillReplies`). 답글은 한 칸 들여쓰고
말풍선 앞에 `↳`가 선다. `lastHumanComment`도 답글까지 훑는다.
([rest.ts](../src/lib/flow/rest.ts), [actions.ts](<../src/app/(app)/actions.ts>),
[thread-view.tsx](../src/components/thread-view.tsx), [api-spec.md](api-spec.md) §13.1·§13.3)

**교훈**: **"응답이 안 변했다"는 최상위 키만 보고 말할 수 없다.** 파라미터를 붙였는데 응답
크기가 늘었으면 늘어난 자리를 찾아야 한다. 그리고 "API가 못 한다"는 결론은 문서에 **어떻게
쟀는지**까지 같이 적어야 한다 — 재는 방법이 틀렸을 때 되짚을 자리가 그것뿐이다.
...
**증상**: `나를 부름` 강조가 세 가지로 틀렸다. (1) 아예 안 붙었다. (2) **내가 쓴 답글**에
붙었다. (3) 2026-07-29 스레드처럼 며칠 지난 멘션에는 안 붙었다.

**원인**: 셋 다 **알림으로 맞춘 탓**이다.

(1)은 죽은 코드를 고쳤다. 알림 → 화면 줄로 옮기는 길이 둘인데(`myMentions`가 사는 길,
`mergeMentionComments`는 아무도 안 부른다), 안 부르는 쪽에 `commentId`를 붙였다. 화면에는
`commentId`가 끝내 안 왔으니 맞출 id가 없었다.

(2)는 flow가 보내는 알림 자체가 어긋난다. 실측 8건 중 1건이 발신자·시각은 남인데 `replyId`는
**내가 쓴 답글**을 가리켰다 (api-spec §7.1).

(3)은 알림을 최근 7일·12건까지만 본다 (`MENTION_DAYS`·`MENTION_LIMIT`). 그 창을 벗어난 멘션은
알림이 없으니 맞출 대상도 없다. **알림은 "요즘 나를 부른 것"의 목록이지, "이 줄이 나를
불렀나"의 답이 아니다.**

**처리**: 판정을 **댓글 본문의 멘션 마크업**으로 옮겼다 (`mentionsMe`). 괄호 안 id가 세션의
`userId`와 같으면 나를 부른 줄이다 — `@[이종석](jongseok.lee@traport.com)`이고,
`/user/employees/me`의 `userId`와 같은 값이다 (실측 2026-08-04). 마크업은 댓글에 영구히 남아서
알림 창과 무관하고, 남이 보낸 엉뚱한 알림도 안 탄다. 전원 호출(`@[ALL](ALL)`)은 id가 `ALL`이라
안 걸리고, 변경 기록은 제외한다 — 담당자로 내 이름이 박힌 기록까지 걸린다. 강조는 멘션 모달과
**업무 상세 모달** 둘 다 켰다. 알림에 붙였던 `commentId`는 도로 걷었다.
([rest.ts](../src/lib/flow/rest.ts), [actions.ts](<../src/app/(app)/actions.ts>),
[thread-view.tsx](../src/components/thread-view.tsx))

**교훈**: **곁다리 신호로 본체를 판정하지 말 것.** "나를 불렀나"는 댓글 자신이 들고 있는데
알림으로 물었다. 알림은 창이 있고(7일·12건) 발신도 못 믿는다 — 창이나 신뢰도가 있는 신호를
판정에 쓰면 그 한계가 그대로 버그가 된다. 그리고 **고치기 전에 그 코드가 살아 있는지부터 볼
것** — 안 불리는 함수를 고치면 증상은 그대로다.
...
**증상**: `내 업무` 화면에 들어가면 화면이 제대로 안 섰다. 머리글·요약 카드·탭 줄까지는
나오는데 그 아래 프로젝트 목록이 **통째로 비어** 있었다. 요약 숫자만 허공에 떠 보였다.
운영에 배포한 뒤에도 같았다.

**원인**: `TabsContent`가 켜진 칸을 `motion.div`로 그리면서 `initial={{ opacity: 0 }}`을
줬다 (beUI 원본 그대로). 이 값은 **서버 HTML에 `style="opacity:0"`으로 박혀 나가고,
하이드레이션이 끝나 `motion`이 붙어야 걷힌다.** 이 화면은 그 칸 하나에 프로젝트 카드
403장·표 60개·DOM 11,542줄이 들어 있어서 하이드레이션이 몇 초 걸리고, 그동안 내용 전체가
투명했다.

허공에 뜬 숫자도 같은 시각의 증상이다. `NumberTicker`는 화면에 들어오기 전까지 `0`을
그리므로(`startOnView`) 하이드레이션 전 요약 카드는 `000`이고, 굴러가는 도중을 보면 자릿수가
군데군데 비어 보인다.

**처리**: 들어오는 움직임을 `motion`에서 **CSS로 내렸다** (`.pane-in`). 지연도 `backwards`도
없어서 애니메이션이 시작하기 전이든 안 돌든 요소는 언제나 제 상태(불투명)다 — JS를 기다리지
않는다. 키가 바뀌면 요소가 새로 붙으므로 탭을 옮길 때 움직임은 그대로다.
([tabs.tsx](../src/components/motion/tabs.tsx), [globals.css](../src/app/globals.css))

실측(로컬, CPU 20배 감속): 내용이 화면에 뜬 시각 16,279ms · 하이드레이션 완료 18,410ms.
그 2.1초가 예전에는 빈 화면이었고 지금은 목록이 보인다. 느린 기기·큰 화면일수록 벌어진다.

**교훈**: **첫 화면을 가리는 일은 JS에 맡기지 말 것.** `initial`은 "나중에 보이게 하겠다"는
약속인데, 그 약속을 지킬 JS가 오기까지가 곧 빈 화면 시간이다. 순수한 등장 연출은 CSS
애니메이션이면 첫 프레임부터 돌고 JS가 늦어도 최악이 "연출 없이 바로 보임"이다. `.rise`가
이미 그렇게 돼 있었는데 탭 칸만 남아 있었다.
...
**증상**: BUG-044를 고친 뒤에도 `내 업무`가 여전히 못 쓸 상태였다. 이번에는 증상이 달랐다 —
화면이 **보이기는 하는데** 그때부터 브라우저 전체가 느려지고, 화면이 깜빡이다가 멈춰 버렸다.
빈 화면이 아니라 **잠긴 화면**이다.

**원인**: `<details>`는 **닫혀 있어도 안쪽이 마운트된다.** 브라우저는 안 그리지만 React는 다
만든다. 이 화면은 카드 40장이 전부 접힌 채로 서는데, 카드마다 업무 표가 하나씩(끝낸 업무까지
치면 둘씩) 달려 있어서 **첫 화면에 표 60개와 DOM 12,000줄**이 생겼다. 표 하나가
`useVirtualizer` 하나라 60개가 한꺼번에 붙는다.

그걸 만드는 동안 메인 스레드가 통으로 잠긴다. 브라우저는 탭 하나가 아니라 **프로세스 단위**로
느려지므로 다른 탭까지 같이 굼떠지고, 힙이 오르면 GC가 끼어들어 화면이 깜빡인다.

**처리**: `WhenOpen` — 조상 `<details>`가 열릴 때까지 안쪽을 안 그린다. 참여자 칸
(`project-panel.tsx`)이 이미 쓰던 방법을 꺼내 쓸 수 있게 만든 것이고, 표를 그 안에 넣었다.
한 번 열리면 계속 남는다 — 비싼 건 40장을 한꺼번에 만드는 것이지 한 장을 들고 있는 게 아니다.
같은 모양이던 리스크 화면의 카드도 같이 고쳤다.
([when-open.tsx](../src/components/when-open.tsx), [tasks/page.tsx](<../src/app/(app)/tasks/page.tsx>),
[risk/page.tsx](<../src/app/(app)/risk/page.tsx>))

실측(로컬 운영 빌드, 사이드바에서 눌러 들어가기, CPU 4배 감속):
...
**교훈**: **접힌 `<details>`는 공짜가 아니다.** `content-visibility`가 레이아웃과 그리기는
건너뛰지만 DOM을 만드는 값과 컴포넌트가 붙는 값은 그대로 낸다. "안 보이니 괜찮다"가 통하는 건
브라우저 쪽 비용뿐이다.

또 하나 — **개발 서버로 잰 성능 수치는 버리는 게 맞다.** 처음에 잰 서버 19초·막힘 790ms는 전부
개발 모드 숫자였다(프로파일에 `jsxDEV`·`diffHydratedProperties`가 그대로 나왔다). 운영 빌드로
다시 재고 나서야 진짜 비용이 보였다.
...
**증상**: 업무 상세에서 상태 줄을 누르면 `요청·진행·피드백·완료·보류`가 나오는데, 그 업무의
지금 상태는 `대기`였다. 다섯 줄 어디에도 체크가 안 선다 — 지금 값이 목록에 없으니 "이 업무가
지금 무슨 상태인지"를 고르기 창이 답하지 못했다.

**원인**: flow가 **같은 상태를 자리마다 다르게 부른다.** `STTS` 코드 `0`이 목록 응답에서는
`대기`(`rest.ts`의 `STTS_LABEL`)고 쓰기 enum에서는 `request`다. 우리 쪽 이름표
(`task-status.ts`의 `TASK_STATUS`)를 쓰기 enum 이름에 가깝게 `요청`으로 적어 놨더니, 표·필터
칩·요약 숫자가 다 `대기`로 그리는 값을 고르기 창만 `요청`으로 그렸다.

체크가 안 서는 것보다 나쁜 결과가 하나 더 있었다. `요청`을 누르면 **지금과 같은 값을 다시
쓰는 요청**이라 flow가 400을 준다. 게다가 낙관적 반영이 표에 `요청`을 꽂아 넣어서, 다른 줄이
전부 `대기`인 표에 혼자 다른 이름이 남는다.

**처리**: 이름표를 화면이 읽는 이름(`대기`)으로 맞췄다. 코드 값(`request`)은 그대로라 쓰기
경로는 안 건드린다. 업무 만들기 확인 문구도 `'대기' 상태로 만들까요?`로 같이 고쳤다.
([task-status.ts](../src/lib/task-status.ts), [new-task-form.tsx](../src/components/new-task-form.tsx))

**교훈**: **한 값에 이름을 둘 두지 말 것.** 읽는 쪽 이름과 쓰는 쪽 이름이 다르면, 둘을 비교하는
자리(지금 값에 체크)가 조용히 틀린다 — 오류도 안 나고 화면만 답을 못 한다.
...
**증상**: 댓글에 답글을 달 때 `@`로 그 사람을 골라 쓰면, 저장된 댓글에 **같은 이름이 두 번**
들어갔다.

**원인**: 답글은 상대를 앞에서 부른 최상위 댓글로 나간다 (REST에 답글 쓰기가 없다 —
[BUG-041](#bug-041)). 그 부르기를 `createComment`가 **조건 없이** 앞에 붙이고 있었다.
본문에서도 자동완성으로 같은 사람을 고르면 `toMentions`가 똑같은 마크업을 한 번 더 만들어
`@[여명호](ymh0510) @[여명호](ymh0510) …`가 된다. 손으로 이름만 친 경우에는 안 겹쳐서
(고른 사람만 마크업으로 바뀐다) 재현 조건이 "자동완성으로 **답할 상대 본인**을 고를 때"로
좁았다.

**처리**: `withCall`로 빼고 이미 부른 글이면 안 붙인다. 서버 액션이 아니라 `thread.ts`에 둔 건
유닛 테스트로 재발을 막기 위해서다. ([thread.ts](../src/lib/thread.ts),
[actions.ts](../src/app/(app)/actions.ts))
...
**증상**: 알림 목록의 한 줄이 **업무인데도** 이 화면에서 안 열렸다. 두 모양이다 — 눌러도
상세 모달 대신 flow 새 창으로 나가거나(`여기서는 못 열어요`), 열리긴 하는데 모달이
`업무가 아닌 글이에요`라고 적고 상태·마감일·담당자 줄이 통째로 없다.

**원인**: 업무인지 아닌지를 **업무명으로 업무 목록을 뒤져서** 판정했다 (`findTaskByPost`) —
`GET /user/posts/projects/{projectId}/tasks/filter?searchWord={업무명}` 첫 장(100건)에서
`postId`가 같은 줄을 고르는 방식이다. 틀리는 길이 셋이다:

| 어긋나는 자리 | 결과 |
|---|---|
| 같은 말이 든 업무가 100건 초과 | 대상이 첫 장 밖 → `업무가 아닌 글` 모달 |
| 제목을 못 푼 줄 (`getPostBrief` 실패) | 검색어가 없어 조회 자체를 건너뜀 → 같은 모달 |
| 필터 조회가 실패 (429·500) | `여기서는 못 열어요` → flow 링크 |

**처리**: 판정 재료를 글 자신에게서 받는다. `GET /user/posts/{postId}` 응답의 `tasks[0]`이
`TASK_SRNO`·`END_DT`·`PRIORITY`를 다 들고 오고, **이 배열이 비어 있다는 게 곧 "업무가 아니다"**
다 — flow가 주는 유일하게 확실한 신호다. 알림 줄은 제목을 얻느라 이 응답을 이미 부르고 있어서
호출 수도 그대로다(1회). `findTaskByPost`는 지웠다.
([rest.ts](../src/lib/flow/rest.ts), [actions.ts](../src/app/(app)/actions.ts))
...
**증상**: 크로미움 계열(웨일·크롬·엣지)에서 `내 업무`에 들어가면 화면이 **나오다 말았다.**
머리글·요약 카드 3장·탭 줄·카드 테두리가 통째로 없고, 업무 표는 머리 줄만 있고 400px쯤
빈 뒤에 줄이 나왔다. 참여자 칸은 `임직원 n명`·`외부 n명` 머리 띠 자리만 비어 있었다.
**사파리는 멀쩡했다.** 매번 났다.

**원인 아닌 것부터** — 이 버그는 세 번 헛짚었고, 그때마다 잰 것이 후보를 하나씩 지웠다.

| 잰 것 | 결과 | 지운 후보 |
|---|---|---|
| 안정된 화면에서 5초 스크롤 | 프레임 평균 8.4ms(약 120fps), p95 9.2ms | 스크롤·렌더 성능 |
| 사이드바 진입을 가로질러 계측 | 최악 프레임 150ms 한 번, 긴 작업 총 186ms | 하이드레이션·메인 스레드 |
| 목 데이터 재현판(38 프로젝트·약 1,000건) | 크로미움 8.4ms / 사파리 16.67ms, 둘 다 정상 | 데이터 양 |

**JS는 처음부터 끝까지 멀쩡했다.** 메인 스레드가 몇 초 잠겼다면 `requestAnimationFrame`이
멈춰 그 공백이 단일 거대 프레임으로 잡히는데, 400ms짜리조차 없었다. 자리는 잡혀 있는데
칠만 빠진 화면 — **합성기(GPU) 쪽**이다.

거기에 무엇을 얹고 있었나. `MorphingModal`이 `viewId`가 `null`이어도 포털을 냈다 — 가드가
`if (!mounted) return null` 하나뿐이라, **닫혀 있어도** `fixed inset-0 z-[100]` 전면 겹이
`document.body`에 남았다. 그 안에 `backdrop-filter: blur(4px)`짜리 배경 단추가 있다
(`opacity:0`·`pointer-events-none`이라 안 보이고 안 눌리지만 **필터는 살아 있다**).

이 껍데기를 `ProjectPanel`이 카드마다 하나씩 들었다 ([v1.8.0](#), 글 모달을 이 화면에서
열게 하면서 `useTaskModal()`이 들어왔다). 포털은 `hidden` 조상을 빠져나가고 `TabsContent`는
안 켜진 탭도 마운트해 두므로, 접힌 카드든 숨은 탭이든 전부 셌다 — **실측 42겹**(프로젝트
38 + 셸 3 + 표 1). 크로미움은 `backdrop-filter`마다 배경을 되읽는 렌더 서피스를 만든다.
42겹이 전면으로 쌓이면 타일 예산이 무너지고, 그 결과가 안 칠해진 화면이다. WebKit은 이
구조를 다르게 잡아서 사파리만 멀쩡했다.

확정은 깨진 화면에서 한 줄로 했다 — `document.querySelectorAll('body > div.fixed.inset-0')
.forEach(n => n.remove())`. 넣는 즉시 제대로 그려졌다.

**처리**: `MorphingModal`이 **닫혀 있으면 포털을 아예 안 낸다.** `open`이 아니라 `live`가
포털의 생사고, `open`이 빠져도 나가는 연출이 끝날 때까지(`AnimatePresence`의
`onExitComplete`) 살려 뒀다가 접는다. 딸려 온 것 둘: 이제 `AnimatePresence`가 패널과 **같이**
붙어서 첫 렌더에 자식이 이미 있으므로 바깥 `initial={false}`를 걷었고(안 걷으면 들어오는
연출이 안 돈다), 배경 겹도 같은 이유로 `initial={{ opacity: 0 }}`을 준다(안 주면 페이드
없이 튄다). ([morphing-modal.tsx](../src/components/motion/morphing-modal.tsx))

**호출자는 한 줄도 안 고쳤다** — `use-task-modal`(카드 38장·검색·알림)·`task-table`·
`mention-table`·`api-key-gate`·`site-footer`가 전부 같이 나았다.

재현판 실측(운영 빌드, 크로미움 151):

| | 진입 | 카드 펼침 | 모달 열림 | Escape 뒤 |
|---|---|---|---|---|
| 전 | 포털 42 · blur 42 | 42 | 42 | 42 |
| 후 | **0 · 0** | 0 (표 25줄 정상) | 1 · dialog · `body.overflow=hidden` | **0** · `visible` |

**교훈**: **안 보이는 것이 공짜가 아니다** — [BUG-045](#bug-045)의 교훈("접힌 `<details>`는
공짜가 아니다")과 같은 자리에 다시 빠졌다. 그때는 DOM을 만드는 값이었고 이번엔 합성 레이어
값이다. `opacity:0` + `pointer-events:none`은 **사람에게만** 없는 것이지 브라우저에게는
있는 것이고, `backdrop-filter`가 붙으면 특히 비싸다. 그리고 하나 더 — **성능 계측이
"정상"이라고 해서 화면이 정상인 건 아니다.** 프레임 시간만 재면 페인트가 통째로 빠진 화면도
120fps로 나온다. 이번엔 스크린샷 한 장이 세 번의 계측보다 빨랐다.
...
**429가 쏟아진다** — 분당 120회다. dev에서 저장할 때마다 화면이 다시 서면 금방 넘긴다.
1분 쉬면 풀린다. 캐시가 살아 있어야 정상 사용량이라 (BUG-040), 이 한계를 재려고 캐시를
끄지 말 것.
