...
| [BUG-034](#bug-034) | 응답에 있는 `upTaskId`·`subTaskCount`를 못 보고 "계층을 못 그린다"로 닫았다 | 문서 정정 (구현은 Phase 8) |
...
**응답에 있는 `upTaskId`·`subTaskCount`를 못 보고 "하위 업무 계층을 못 그린다"로 닫았다** —
2026-07-31, 문서 정정 (구현은 Phase 8)
...
**처리**: PRD §6.5 한계 ①·§13 D1·Phase 8을 고쳤다. 구현은 아직 안 했다 — 부모까지 내 목록에 있는
건이 191건 중 26건뿐이라, 들여쓰기는 그 26건에만 붙이고 나머지는 평평하게 둔다. 없는 부모를 받으려면
건당 조회 165회가 필요하고 분당 120회 예산에 걸린다.
