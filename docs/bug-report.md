...
| ID | 대상 | 상태 |
|----|------|------|
| [BUG-001](#bug-001) | flow MCP `flow_list_alarms` | 없어짐 (v4.0.0 — MCP 폐기) |
...
| [BUG-007](#bug-007) | flow MCP `flow_list_projects` | 없어짐 (v4.0.0 — MCP 폐기) |
| [BUG-008](#bug-008) | `"use server"` 모듈의 상수 export 소실 | 해결 |
| [BUG-009](#bug-009) | 아코디언 본문 `-mx-5`로 UI 잘림 | 해결 |
| [BUG-010](#bug-010) | 헤더 블러가 눈에 안 보였다 | 해결 |
| [BUG-011](#bug-011) | `flow_collect_project_chain` 권한 오류 | 없어짐 (v4.0.0 — MCP 폐기) |
...
| [BUG-034](#bug-034) | 응답에 있는 `upTaskId`를 못 보고 "계층을 못 그린다"로 닫았다 | 해결 (v1.3.0) |
| [BUG-035](#bug-035) | `systemCode`가 채워졌으면 시스템 댓글로 봤다 — 사람 댓글 38%가 사라졌다 | 해결 (v1.4.0) |
...
| [BUG-040](#bug-040) | 분당 120회를 넘겨 프로젝트 59개 중 26개가 조용히 빠졌다 | 해결 (v4.0.0 · v4.1.0) |
| [BUG-041](#bug-041) | REST에는 답글이 없다 — 답글이 최상위 댓글로 붙었다 | 해결 (v4.0.0) |
| [BUG-042](#bug-042) | 쿼리를 붙이고도 응답의 새 필드를 안 봐서 답글이 통째로 빠졌다 | 해결 (v4.2.0) |
| [BUG-043](#bug-043) | 나를 부른 댓글 강조를 알림으로 맞춰 옛 멘션이 빠지고 내 말이 걸렸다 | 해결 (v4.2.0) |
| [BUG-044](#bug-044) | `내 업무`가 하이드레이션 전까지 `opacity:0`으로 통째로 비어 보였다 | 해결 (v4.8.0) |
| [BUG-045](#bug-045) | 화면은 뜨는데 브라우저가 느려지다 멈췄다 | 해결 (v4.8.0) |
...
| [BUG-051](#bug-051) | 크로미움에서 `내 업무` 화면이 통째로 안 그려졌다 — 닫힌 모달 42겹이 합성기를 무너뜨렸다 | 해결 (v4.17.2) |
...
**응답에 있는 `upTaskId`를 못 보고 "하위 업무 계층을 못 그린다"로 닫았다** — 2026-07-31, 해결 (v1.3.0)

v1.2.0에서 `/tasks`를 만들 때 계층을 포기하고 PRD §6.5 한계 ①에 이렇게 적었다: "`upTaskName`이 전부
...
**처리**: PRD §6.5 한계 ①·§13 D1·Phase 8을 고치고 **v1.3.0에서 구현했다**. 부모까지 내 목록에 있는
...
**`systemCode`가 채워졌으면 시스템 댓글로 봤다 — 사람 댓글 38%가 사라졌다** — 2026-07-31, 해결 (v1.4.0)
...
**`revalidatePath`에 쿼리스트링을 넘겨 캐시가 안 지워졌다** — 2026-07-31, 해결 (v1.4.0)
...
**모달에서 바꾼 값이 업무 줄에 6.5초 뒤에야 닿았다** — 2026-07-31, 해결 (v1.4.1)
...
**우선순위·담당자를 한 번 못 가져오면 새로고침까지 잠겼다** — 2026-07-31, 해결 (v1.4.2)
...
> **반쪽이었다 (v4.1.0)**. 위 2번을 오늘·내 업무 두 화면에만 붙였고, **팀·리스크는 `loadTeam`이
...
**상태**: 해결 (v4.0.0) · 팀·리스크 화면까지 확장 (v4.1.0)

---

## BUG-041

**증상**: v2.2.0에 붙인 답글이 REST로는 안 된다. 답글을 달면 스레드가 아니라 **최상위 댓글**로
붙는다.
...
**상태**: 해결 (v4.0.0) — 스레드 자체는 flow REST가 열어 주기 전까지 못 만든다.

> **읽기는 열려 있었다 (v4.2.0)**: [BUG-042](#bug-042). 여기서 "못 만든다"고 적은 건
...
이 껍데기를 `ProjectPanel`이 카드마다 하나씩 들었다 ([v4.16.0](#), 글 모달을 이 화면에서
