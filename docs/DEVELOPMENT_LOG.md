...
### 2026-08-07 — 참여자 칸 바닥값을 실측으로 다시 잡았다 (v4.17.4)

`수정` **v4.17.3의 바닥값 256px이 모자랐다.** 업무가 한두 건인 프로젝트는 왼쪽 열이 짧아서
...
`changelog.ts`에는 v4.17.3을 따로 안 남겼다 — 같은 변화의 같은 날 다듬기라 사용자 쪽에는
v4.17.4 한 줄로 합쳤다.
...
### 2026-08-07 — 참여자 칸이 높이를 채우고, 댓글 수는 제목 옆으로 (v4.17.3)
...
### 2026-08-07 — 닫힌 모달은 자리를 안 차지한다 (v4.17.2)
...
그걸 하나씩 들어서(v4.16.0에 `useTaskModal()`이 들어왔다) 진입 시 **42겹**이 쌓였다. 크로미움은
...
### 2026-08-07 — 업무 아닌 글을 표 아래로 내리고, 조회 실패 사유를 남긴다 (v4.17.1)
...
코드 동작 변화 없음(주석·문서만). 버전은 그대로 v4.16.3이다.
...
### 2026-08-06 — 업무가 아닌 글도 우리 화면에서 읽는다 (v4.16.0)
...
### 2026-08-06 — 상태·우선순위 줄을 손보고, 답글이 사람을 두 번 부르던 것을 고친다 (v4.15.1)
...
### 2026-08-06 — 안 쓰던 flow 쓰기 경로 넷을 붙인다 (v4.15.0)
...
### 2026-08-06 — 우선순위가 숫자로 보이던 것을 고친다 (v4.14.2)
...
### 2026-08-06 — 부를 사람 목록이 부르는 자리에 선다 (v4.14.1)
...
### 2026-08-06 — 부른 이름이 진짜로 부른다 (v4.14.0)
...
### 2026-08-06 — 기다리는 자리를 걷어낸다 (v4.13.2)
...
### 2026-08-05 — 골격이 도착할 화면과 같은 키가 된다 (v4.13.1)

`개선` **로딩 골격을 지금 화면에 맞춰 다시 잰다.** 골격의 규칙은 하나다 — *틀은 진짜, 글자만
회색 막대.* 그런데 v4.4.0~v4.13.0 사이에 화면이 여러 번 바뀌는 동안 골격은 그대로여서, 내용이
...
- **리스크 · 내 업무의 접힌 카드** — 한 부품(`SummaryCardsSkeleton`)이 두 화면을 같이 그렸는데
  둘의 모양이 v4.4.0·v4.7.0에 갈라졌다. 리스크는 등급 점·이름·통계 한 줄 + 점수 막대 + 담당자
...
- **폰의 탭 줄** — `sm` 아래에서 진짜 탭은 고르개 한 줄이라 가로를 다 쓴다(v4.13.0). 골격만
...
### 2026-08-05 — 부른 이름은 굵게, 답은 그 자리에 (v4.13.0)
...
### 2026-08-05 — 선과 면을 제자리에 놓는다 (v4.12.1)

`개선` **`neutral` 요약 칸도 면을 받는다.** v1.7.0에 요약 카드에 면(`face` — 위→아래
...
### 2026-08-05 — 새 소식이 오면 OS 알림을 띄운다 (v4.12.0)
...
### 2026-08-05 — 검색 팔레트가 네 갈래를 찾는다 (v4.11.0)
