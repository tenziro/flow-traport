...
- **계정** — 레일 발에 flow 프로필 사진(없으면 `👋🏻`)과 이름·부서·로그인한 이메일을 낸다.
  마우스를 올리면(또는 Enter) 상태 메시지와 오늘 일정·로그아웃이 든 팝오버가 옆으로 열린다.
  좁은 화면은 헤더의 로그아웃 단추 하나뿐이다.
- **오늘 일정 서랍** — 계정 팝오버의 "오늘 일정"을 누르면 오른쪽에서 서랍이 들어온다. 오늘
  화면 카드와 같은 하루를 같은 줄 모양으로 내서, 내 업무·리스크·팀·구성원에서도 오늘 흐름을
  본다. 건수는 단추에 함께 적힌다. 좁은 화면에는 레일이 없어서 이 길도 없다.
...
### 2026-08-03 — 오늘 일정을 계정 팝오버의 서랍으로 (v1.6.0)

`기능` 오늘 일정(§13 B3)을 오늘 화면 밖에서도 연다. 레일 발의 계정 팝오버(§7.3)에 "오늘 일정"
단추를 넣었고, 누르면 오른쪽에서 서랍이 들어온다. 오늘 화면 4단 카드는 그대로 둔다 — 거기서는
그 카드가 하루의 마지막 문단이고, 서랍 뒤로 숨기면 하루가 업무 목록에서 끝난다. 소식(종)처럼
헤더로 올리지 않은 것은 종이 "새 것이 왔다"는 알림이고 일정은 내가 찾아보는 것이라, 늘 보이는
자리를 하나 더 내줄 이유가 없어서다. 대신 건수만 단추에 함께 적어서 열지 않고도 빈 날인지 안다.
...
데이터는 셸이 받는다 — `loadTodayEvents()`를 `src/lib/flow/queries.ts`에 두고 React `cache()`로
감쌌다. `loadToday()`도 같은 함수를 부르므로 `/` 한 요청에서 REST 왕복은 하나다 (dev 로그 계측:
요청 1회에 실호출 1회). 인자를 안 받는 것은 부르는 쪽마다 `Date.now()`가 달라지면 캐시 키가
갈리기 때문이다.

좁은 화면(<1024px)에는 이 서랍이 없다. 레일이 없어서 계정 팝오버 자체가 없고, 거기서는 오늘
일정을 오늘 화면 카드로 본다.
...
`src/lib/flow/queries.ts`, `src/app/(app)/layout.tsx`, `docs/PRD.md`(§6.1·§7.3),
