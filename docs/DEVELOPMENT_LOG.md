...
| `/tasks` (내 업무) | 담당 업무 전량(실측 951건)을 프로젝트 아코디언으로. 세 무리를 탭으로 갈랐다. 접힌 카드는 프로젝트 요약(설명·중요·참여자 수·공개 여부·개설 정보), 펼치면 건수·막대·업무 표에 오른쪽 참여자 칸과 **업무 아닌 글**(공지·회의록·일정) 목록 — 그 제목을 누르면 이 화면에서 글 모달이 열린다. 하위 업무는 상위 업무 아래로 들여쓴다. 끝낸 업무도 같은 표고, 안 끝난 게 있을 때만 접어 둔다 | 운영 |
