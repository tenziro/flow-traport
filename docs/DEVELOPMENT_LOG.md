...
- **업무 표** — 오늘·리스크·팀·내 업무 네 화면이 같은 표 한 벌을 쓴다(`TaskTable`).
  ...
  붙인다(안 읽은 게 남았으면 꽉 찬 색). 업무명 **앞**에는 하위 업무 화살표와 우선순위
...
- **댓글** — 상세 모달과 멘션 패널에서 전문을 읽고 그 자리에서 남긴다. 상세 모달은 최신
  최상위 댓글 두 개와 거기 딸린 답글 전부를 펼쳐 두고 나머지는 갯수 줄 오른쪽 끝
  `댓글 다 보기`로 연다 — 댓글의 태반이 상태 변경
  기록이다(시스템 기록은 되감기 아이콘). **나를 부른 사람들 모달은 열자마자 전량을 편다.
