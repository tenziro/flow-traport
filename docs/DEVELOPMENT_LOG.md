...
- **업무 표** — 오늘·리스크·팀·내 업무 네 화면이 같은 표 한 벌을 쓴다(`TaskTable`).
  프로젝트·업무명·진행상태·마감일 네 칸이고 내 업무 화면만 등록일이 더 붙는다(그 응답에만
  있는 값이다). 리스크는 프로젝트 대신 담당자 칸을 켠다. 칸 이름을 눌러 정렬하고, 표 위
  상태 칩으로 화면을 다시 안 부르고 거른다. 나를 부른 사람들만 칸이 다르다 —
  프로젝트·업무명·부른 사람·마지막 말·시각.
- **업무 상세 모달** — 표에서 업무명을 누르면 열린다. 추천 이유·점수·마지막 댓글을 읽고
  상태·마감일·우선순위·담당자를 flow로 나가지 않고 고친다(담당자는 여러 명을 켠다).
